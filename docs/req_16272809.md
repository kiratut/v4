...
> Экспортировано из Excel req.xlsx (дата: 28.09.2025 16:27)
...
Test ID (alt):
Рекомендация по тесту:Дополнить тест test_resource_monitoring_critical_thresholds проверкой расчета среднего потребления ресурсов за период, как указано в расширенном описании требования.
=== ROW ===
...
Test ID (alt):
Рекомендация по тесту:Текущий тест покрывает требование. Рекомендаций нет.
=== ROW ===
...
Test ID (alt):
Рекомендация по тесту:Спорный момент. Тест test_02_api_auth_headers является поверхностным. Необходимо создать новый интеграционный тест, который проверяет реальное снижение % рабочих профилей при имитации ошибок, как описано в требовании.
=== ROW ===
...
Техническая информация:[System_Revision_Report_archived.md] 
Test ID (alt):
Рекомендация по тесту:Текущий тест проверяет наличие записей в логе. Рекомендуется добавить сценарий, который имитирует "зависшую" задачу и проверяет, что функция _check_timeouts корректно ее обнаруживает.
=== ROW ===
...
Test ID (alt):test_cleanup_command
Рекомендация по тесту:Модели нашли тест, но он проверяет только конфигурацию. Необходимо создать интеграционный тест, который реально создает старые логи (файловые и в БД) и проверяет их фактическое удаление/архивацию.
=== ROW ===
...
Техническая информация:[V4_RUNBOOK.md] 
Test ID (alt):test_cleanup_command
Рекомендация по тесту:Аналогично предыдущему пункту, существующий тест не проверяет фактическое удаление архивов. Требуется интеграционный тест с созданием и удалением тестовых архивов.
=== ROW ===
...
Test ID (alt):
Рекомендация по тесту:Создать интеграционный тест, который добавляет в тестовую БД "старые" вакансии и проверяет, что они корректно архивируются, а лишние поля очищаются.
=== ROW ===
...
Техническая информация:[Regular_Procedures_v4.md] 
Test ID (alt):
Рекомендация по тесту:Спорный момент. Тест test_cleanup_command проверяет только конфиг. Необходимо создать интеграционный тест, который имитирует превышение лимита архивов и проверяет их удаление и отправку уведомления.
=== ROW ===
...
Техническая информация:[Regular_Procedures_v4.md] 
Test ID (alt):
Рекомендация по тесту:Создать тест, который наполняет БД старыми записями индикаторов и проверяет их корректное удаление в соответствии с параметрами.
=== ROW ===
...
Техническая информация:[System_Revision_Report_archived.md] 
Test ID (alt):
Рекомендация по тесту:Текущий тест покрывает базовые требования. Можно расширить его для проверки корректности формата записей в логах.
=== ROW ===
...
Техническая информация:[Requirements_Refinement_Analysis.md] 
Test ID (alt):
Рекомендация по тесту:Добавить в существующие тесты (например, test_critical_event_logging) ассерты, которые проверяют наличие и корректность префиксов модулей и имен функций в записях логов (как в файлах, так и в БД).
=== ROW ===
...
Техническая информация:[Requirements_Refinement_Analysis.md] 
Test ID (alt):
Рекомендация по тесту:Создать unit-тест, который использует mock-данные для генерации Excel-файла и проверяет корректность его структуры и содержимого.
=== ROW ===
Requirement ID:2.4
...
Техническая информация:[V4_RUNBOOK.md] 
Test ID (alt):
Рекомендация по тесту:Спорный момент. Тест test_dispatcher_start_command проверяет только инициализацию объекта. Необходимо создать полноценный интеграционный тест, который проверяет запуск/остановку реального процесса демона через CLI.
=== ROW ===
...
Test ID (alt):
Рекомендация по тесту:Спорный момент. Тест проверяет доступность веб-интерфейса, но не проверяет ключевую часть требования\: автоматический запуск/остановку панели вместе с демоном. Требуется интеграционный end-to-end тест.
=== ROW ===
...
Техническая информация:[System_Revision_Report_archived.md] 
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Модели не нашли тест test_system_health_task. Необходимо либо найти этот тест, либо создать новый, который проверяет периодичность вызова диагностики и отправку уведомлений в Telegram при тревогах.
=== ROW ===
...
Техническая информация:[Dashboard_Specification_v4.md] 
Test ID (alt):
Рекомендация по тесту:Тест проверяет обновление времени. Рекомендуется дополнить его, имитируя устаревшие данные в БД и проверяя, что они действительно окрашиваются в специальный цвет на панели (требуется UI-тест, например, с Playwright).
=== ROW ===
...
Дата:21.09.2025 
Техническая информация:[System_Revision_Report_archived.md] 
Test ID (alt):test_dispatcher_start_command
Рекомендация по тесту:Критическое расхождение. Модели не нашли тест test_task_manager_basic_operations. Требование комплексное, и текущие тесты покрывают его крайне слабо. Необходимо создать набор unit- и интеграционных тестов для всех аспектов диспетчера (очередь, ETA, таймауты).
=== ROW ===
...
Техническая информация:
Test ID (alt):
Рекомендация по тесту:(Требование без приоритета)
=== ROW ===
Requirement ID:2.5
...
Дата:21.09.2025 
Техническая информация:[Dashboard_Specification_v4.md] 
Test ID (alt):test_stats
Рекомендация по тесту:Модели нашли интеграционный тест API, но он проверяет только доступность эндпоинта. Необходимо создать unit-тесты для функций analyze_logs и get_stats с mock-данными для проверки корректности расчетов.
=== ROW ===
...
Дата:21.09.2025 
Техническая информация:[Dashboard_Specification_v4.md] 
Test ID (alt):test_status_indicators
Рекомендация по тесту:Расхождение. Эталон ссылается на test_system_health_check, модели — на UI-тест. Необходимо убедиться, что оба аспекта (сбор данных и их отображение) покрыты, и, возможно, унифицировать тесты.
=== ROW ===
...
Техническая информация:[Dashboard_Specification_v4.md] 
Test ID (alt):
Рекомендация по тесту:Тест проверяет запись в логе. Это косвенная проверка. Рекомендуется создать UI-тест, который нажимает кнопку и проверяет, что данные на панели действительно обновились.
=== ROW ===
...
Техническая информация:[Dashboard_Specification_v4.md] 
Test ID (alt):
Рекомендация по тесту:Создать интеграционный тест, который изменяет параметр частоты через API или напрямую в конфиге, а затем проверяет (через логи или mock-объекты), что планировщик задач отреагировал и изменил расписание.
=== ROW ===
...
Техническая информация:[Dashboard_Specification_v4.md] 
Test ID (alt):
Рекомендация по тесту:Создать полноценный UI-тест (Playwright), который проверяет весь цикл\: отображение фильтров из файла, изменение их состояния через кнопки ("вкл все"), сохранение изменений обратно в filters.json.
=== ROW ===
Requirement ID:2.6
...
Test ID (alt):
Рекомендация по тесту:Текущий тест проверяет наличие ключей в конфиге. Рекомендуется создать интеграционный тест с mock-сервером Telegram API для проверки реальной отправки сообщений.
=== ROW ===
...
Test ID (alt):test_02_api_auth_headers
Рекомендация по тесту:Критическое расхождение. Модели не нашли тест test_hh_multiple_auth_profiles. Найденный тест test_02_api_auth_headers проверяет только наличие профилей, но не логику их применения. Требуются полноценные тесты для core/auth.py.
=== ROW ===
...
Test ID (alt):test_dispatcher_start_command
Рекомендация по тесту:Критическое расхождение. Аналогично 2.4.5, в эталоне указан тест, который модели не нашли. Требуется расширение тестов для core/task_dispatcher.py.
=== ROW ===
...
Test ID (alt):
Рекомендация по тесту:Создать тест, который проверяет применение всех настроек логирования из config_v4.json\: уровень, ротацию файлов, формат записей.
=== ROW ===
...
Test ID (alt):test_resource_monitoring_critical_thresholds
Рекомендация по тесту:Важный спорный момент. Модель gpt5-hr обнаружила, что тест использует ключи system_monitoring, а требование — health_check.*. Необходимо синхронизировать имена ключей в коде и тестах, чтобы избежать ложно-положительных результатов.
=== ROW ===
...
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Все ТОП-модели единодушны, что тестов для модуля core/auth.py нет. Необходимо в первую очередь создать unit-тест для функции test_profile.
=== ROW ===
...
Техническая информация:[config/auth_roles.json] 
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Необходимо создать unit-тест для функции get_profile_for_task, имитируя различные задачи и проверяя корректность выбора профиля.
=== ROW ===
...
Техническая информация:[Requirements_Refinement_Analysis.md] 
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Необходимо создать unit-тест для функции rotate_on_error, имитируя ошибки API и проверяя логику переключения профилей.
=== ROW ===
...
Техническая информация:[Requirements_Coverage_Report.md] 
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Требуется создать интеграционный тест для test_auth_params, который использует mock HH API для проверки логики перебора параметров.
=== ROW ===
Requirement ID:2.9
...
Техническая информация:[Database_Schema_v4.md] 
Test ID (alt):
Рекомендация по тесту:Расхождение. Создать тест, который запускает benchmark_queries и проверяет, что результаты (время выполнения) корректно записываются в лог или возвращаются.
=== ROW ===
...
Техническая информация:[Requirements_Coverage_Report.md] 
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Создать полноценный интеграционный тест для save_vacancy, который проверяет корректное сохранение, дедупликацию и версионирование.
=== ROW ===
...
Техническая информация:[Database_Schema_v4.md] 
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Создать интеграционный тест для get_vacancy, который проверяет корректное извлечение данных, сохраненных на предыдущем шаге.
=== ROW ===
...
Техническая информация:[Requirements_Coverage_Report.md] 
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Создать интеграционный тест для CLI-команды export, который генерирует файл и проверяет его содержимое.
=== ROW ===
...
Техническая информация:[Requirements_Coverage_Report.md] 
Test ID (alt):
Рекомендация по тесту:Расхождение. Создать unit-тест для get_statistics с тестовой БД для проверки корректности расчетов.
=== ROW ===
...
Техническая информация:[Regular_Procedures_v4.md] 
Test ID (alt):
Рекомендация по тесту:Расхождение. Создать интеграционный тест для cleanup_old_records, который наполняет БД старыми записями и проверяет их удаление.
=== ROW ===
Requirement ID:2.11
...
Техническая информация:
Test ID (alt):
Рекомендация по тесту:Спорный момент. Текущий тест является интеграционным. Рекомендуется создать дополнительный unit-тест для функции build_search_request, чтобы изолированно проверить логику построения URL.
=== ROW ===
...
Test ID (alt):
Рекомендация по тесту:Расхождение. Создать unit-тест для calculate_pagination, используя mock-ответы от HH API для проверки корректности расчетов страниц.
=== ROW ===
...
Техническая информация:[Requirements_Test_Catalog.md] 
Test ID (alt):
Рекомендация по тесту:Создать интеграционный тест, который проверяет, что после сбора ID вакансий они корректно добавляются в очередь в БД функцией save_vacancy_queue.
=== ROW ===
Requirement ID:2.12
...
Техническая информация:[Requirements_Test_Catalog.md] 
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Создать интеграционный тест для fetch_vacancy_details, используя mock HH API для проверки загрузки и парсинга полной информации о вакансии.
=== ROW ===
...
Техническая информация:[Requirements_Coverage_Report.md] 
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Создать unit-тест для check_duplicate, который проверяет логику дедупликации на заранее подготовленном наборе данных (с дублями и без).
=== ROW ===
...
Техническая информация:
Test ID (alt):
Рекомендация по тесту:Расхождение. Создать unit-тест для enrich_vacancy_data, чтобы проверить корректность расчета дополнительных полей.
=== ROW ===
...
Техническая информация:[Requirements_Coverage_Report.md] 
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Создать интеграционный тест для save_with_versioning, который проверяет создание новой версии для существующей вакансии.
=== ROW ===
...
Test ID (alt):
Рекомендация по тесту:Создать интеграционный тест, который проверяет, что после успешной обработки вакансии соответствующая задача удаляется из очереди функцией remove_from_queue.
=== ROW ===
Requirement ID:2.13
...
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Создать интеграционный тест, который использует mock HH API для имитации ошибок 5xx/429 и проверяет, что логика ExponentialBackoff работает корректно (делает повторные запросы с увеличивающейся задержкой).
=== ROW ===
...
Техническая информация:[Regular_Procedures_v4.md] 
Test ID (alt):test_resource_monitoring_critical_thresholds
Рекомендация по тесту:Расхождение. Текущий тест проверяет только сбор метрики. Необходимо создать тест, который имитирует состояние "мало места на диске" и проверяет, что система корректно деградирует (например, останавливает тяжелые задачи).
=== ROW ===
...
Техническая информация:[System_Revision_Report_archived.md] 
Test ID (alt):
Рекомендация по тесту:Критическое расхождение. Создать интеграционный тест, который имитирует потерю соединения с БД (например, останавливая тестовый контейнер с БД) и проверяет, что функция get_connection способна успешно переподключиться.
