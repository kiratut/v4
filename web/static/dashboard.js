...
function createMainCard(cardConfig) {
    ...
    if (content) {
        ...
        // Таблица фильтров
        if (content.filters_table) {
            const filtersDiv = document.createElement('div');
            filtersDiv.style.borderTop = '1px solid #eee';
            filtersDiv.style.paddingTop = '8px';
            filtersDiv.style.marginTop = '8px';

            const tableTitle = document.createElement('div');
            tableTitle.style.fontSize = '12px';
            tableTitle.style.marginBottom = '4px';
            tableTitle.style.fontWeight = '600';
            tableTitle.textContent = 'Filters Table';
            filtersDiv.appendChild(tableTitle);

            // Создаем таблицу
            const table = document.createElement('table');
            table.id = content.filters_table.id || 'filtersTable';
            ...
            (content.filters_table.columns || []).forEach(col => {
            ...
            // Кнопки управления
            if (content.filters_table.controls) {
                const controlsDiv = document.createElement('div');
                controlsDiv.style.display = 'grid';
                controlsDiv.style.gridTemplateColumns = 'repeat(auto-fit, minmax(100px, 1fr))';
                controlsDiv.style.gap = '4px';
                controlsDiv.style.marginTop = '8px';

                content.filters_table.controls.forEach(control => {
                    const btn = document.createElement('button');
                    btn.textContent = control.text;
                    btn.title = control.tooltip || '';
                    btn.onclick = () => eval(control.action);
